 ...
 * 
 * Query parameters:
 * - search: string (optional) - search query for user name/email
 * - role: string (optional) - filter by role (farmer, mycologist, admin, etc.)
...
export async function GET(req: NextRequest) {
  try {
    ...
    const upstreamUrl = new URL(`${envOptions.apiUrl}/user/search`);
    if (searchQuery) upstreamUrl.searchParams.set('search', searchQuery);
    if (role) upstreamUrl.searchParams.set('role', role);
    if (active !== undefined) upstreamUrl.searchParams.set('active', String(active));
    if (limit) upstreamUrl.searchParams.set('limit', limit);
    if (pageToken) upstreamUrl.searchParams.set('pageToken', pageToken);

    console.log('📤 GET /api/v1/users/search proxy');
    console.log('  Query params:', { searchQuery, role, active, limit, pageToken: pageToken ? `${pageToken.substring(0, 20)}...` : 'NONE' });
    ...
    console.error('GET /api/v1/users/search proxy error', err);

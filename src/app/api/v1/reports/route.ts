...
export async function GET(req: NextRequest) {
  try {
    ...
    const upstreamUrl = new URL(`${envOptions.apiUrl}/report`);
    upstreamUrl.searchParams.set('limit', limit);
    if (pageToken) {
      upstreamUrl.searchParams.set('pageToken', pageToken);
    }

    console.log('📤 GET /api/v1/reports proxy');
    ...
    console.error('GET /api/v1/reports proxy error', err);
